...
class InputField{
    ...
    set errors(errors){
        ...
        errorToRemove.forEach(fn=> {
            Vue__default['default'].delete(this._errors,fn);
        });
        errorToAdd.forEach(err=> {
            Vue__default['default'].set(this._errors,err.fn,err.error);
        });   
    }
    ...
    async validate(trigger){
        ...
        //needed it should run after setFormat() takes affect
        ...
        if(trigger && this.dirty){ 
            ...
            errors = this.rules
                .filter(rule => {
                  return typeof rule.trigger == 'object' ?
                        rule.trigger.find(trig=> trig == trigger)
    ...
    async setFormat(trigger,e){
        ...
        const hasTrigger = (trigger)=> {
            return typeof this.format.trigger == 'object' ?
                this.format.trigger.includes(trigger)
